...
| Optional tools, checked where they are needed | **done** | A feature that needs a tool Flight Deck does not ship says so at the moment you use it, not during `npm install` — there is no postinstall hook and a test keeps it that way. The page names what is missing, hands you the official command *for this machine* (winget/choco/scoop/brew/dnf/pacman, whichever answered), types it into your own terminal where you can watch it, and re-checks on demand. Installed and signed-in are treated as different states. Nothing is ever installed in the background. |
| Sign in to GitHub by pasting a token | **done** | The primary sign-in path: a link that opens GitHub's token page with the right scopes ticked, a field, and a button. The token goes to `gh` on stdin and gh keeps it in the system credential store — Flight Deck stores nothing. The CLI's own `gh auth login` is still offered, demoted, for anyone who prefers it. |
| One-time codes out of the terminal | **done** | A device-code login (`gh auth login`, and every other CLI that does this) prints a code and waits — and a terminal is the one place you cannot casually copy from, since `Ctrl+C` is SIGINT and kills the flow. The code is read out of the output, escape sequences and chunk boundaries included, and offered as a banner: copy it, or copy-and-press-Enter so the CLI opens the browser itself. A link to whatever host the output named is the fallback. |
| Review a branch before you raise it | **done** | The agent reads your change against the branch this project's pull requests go to — committed *and* uncommitted, plus untracked files no diff contains — and returns findings anchored to file and line, worst first, each expanding to its reasoning. It fetches the diff itself and reads whole files for context rather than being handed a truncated patch, and it runs in the CLI's plan mode so it cannot edit anything. Needs no GitHub: works for every project, any host, even one with no remote. |
| Open pull requests, across every project | **done** | Grouped by project, most recently active first, with the project you are in kept at the top and quiet ones listed rather than dropped — "nothing open" is an answer. Never filtered by target branch. Each repository comes from that project's own `origin` — never a browser of every repo on the account. Each row: number, title, author, `head → base`, files changed with additions and deletions, draft and changes-requested state, when it moved, and a link out. Every failure is a state that explains itself, including GitHub's deliberately ambiguous "exists or you cannot see it". |
| Review a pull request | **done** | Select one from the list and read it: title, author, `head → base`, counts, and its full diff in the same viewer the Changes panel uses. Reviewing fetches its commit into `refs/flightdeck/pr-N` — no checkout, no branch switch, no working-tree change — so the reviewer reads whole files as the pull request leaves them rather than guessing from a patch. |
| Review page layout | **done** | A list and a detail pane, resizable, rather than one stacked card: the list stays scannable while a 600-line diff gets the width. Branch review sits at the top of the list, ungated. |
| Pull requests page | **partly** | Posting selected findings back as a pending review is the remaining piece — see DECISIONS.md for why line comments are last. |
...
**57 features done. Nothing left in P1 or P2 as a whole feature** — the remaining work is six partial rows,
